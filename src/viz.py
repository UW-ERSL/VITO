...
def plot_grid_contour(
  ...
  levels: int = 50,
  binary_fill: bool = False,
  val_range: tuple[float, float] = None,
  label: str = None,
  colorbar: bool = True,
  upsample_factor: int = 1,
) -> plt.Axes:
  """Plot a smoothed contour representation of an element field on a structured grid.

  Args:
    mesh: The mesh object.
    field: Elementwise field to plot. Must have shape (mesh.num_elems,).
    ax: The matplotlib axes to plot on. If None, a new figure is created.
    cmap: The colormap to use.
    contour_level: The contour value used to draw the boundary.
    contour_color: Color of the contour line.
    contour_linewidth: Width of the contour line.
    levels: Number of contourf levels if binary_fill is False.
    binary_fill: If True, fill with only two colors split at contour_level.
    val_range: Optional tuple of (min_val, max_val) for contourf range.
    label: The label for the colorbar. If None, no label is added.
    colorbar: Whether to add a colorbar.

  Returns:
    ax: The matplotlib axes with the plotted contour field.
  """
  if ax is None:
    _, ax = plt.subplots()
  ...
  # infer structured grid dimensions from element centers
  elem_centers = mesh.elem_centers
  x_unique = np.unique(elem_centers[:, 0])
  y_unique = np.unique(elem_centers[:, 1])

  nelx = len(x_unique)
  nely = len(y_unique)

  assert nelx * nely == mesh.num_elems, (
    "plot_grid_contour currently assumes a structured grid."
  )

  # reshape element field to 2D grid
  field_2d = np.asarray(field).reshape(nely, nelx)

  if upsample_factor > 1:
    field_2d = zoom(field_2d, upsample_factor, order=3)
    xv = np.linspace(x_unique.min(), x_unique.max(), field_2d.shape[1])
    yv = np.linspace(y_unique.min(), y_unique.max(), field_2d.shape[0])
  else:
    xv = x_unique
    yv = y_unique
  ...
  if binary_fill:
    fmin = np.min(field_2d)
    fmax = np.max(field_2d)
    if fmin == fmax:
      fmin -= 0.5
      fmax += 0.5

    # use only the endpoint colors of the passed colormap
    base_cmap = plt.get_cmap(cmap) if isinstance(cmap, str) else cmap
    cmap_binary = ListedColormap([base_cmap(0.0), base_cmap(1.0)])

    cf = ax.contourf(
      X, Y, field_2d,
      levels=[fmin, contour_level, fmax],
      cmap=cmap_binary,
    )
  else:
    if val_range is not None:
      ...
      if fmin == fmax:
        fmin -= 0.5
        fmax += 0.5
      contour_levels = np.linspace(fmin, fmax, levels)

    cf = ax.contourf(X, Y, field_2d, levels=contour_levels, cmap=cmap)
